...
public class VarLong : INetworkType
{
    ...
    private const int MAX_READER_POSITION_OFFSET = 64;
    ...
    public VarLong(long value = 0)
    {
        InternalBuffer = new byte[VARINT_MAX_ARRAY_ELEMENT_COUNT];
        InternalValue = value;
    }

    public static VarLong FromStream(Stream stream)
    {
        long value = 0;
        int position = 0;
        int read;
    ...
    public void ReadFromStream(Stream stream)
    {
        ...
        while ((read = stream.ReadByte()) != END_OF_STREAM)
        {
            ...
            if (position >= MAX_READER_POSITION_OFFSET)
            {
                throw new InvalidOperationException("VarInt exceeded the 64-bit signed integer value limit.");
            }
